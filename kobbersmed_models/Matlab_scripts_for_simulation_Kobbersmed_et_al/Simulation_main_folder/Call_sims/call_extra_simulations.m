function call_extra_simulation() %(model_type, par_free)
...
model_type = 46;
par_free(1) = 13.7718;
par_free(2) = 1; %Variable but constant unpriming rate
par_free(3) = 0; %Variable but constant unpriming rate
par_free(4) = 134.8542;
par_free(5) = NaN;
par_free(6) = 180;
...
for unprim_const = [0 50 0.7247*236.82 1000]
    par_free(5) = unprim_const;
    [par_init, savefilename] = parameter_choices(par_free, model_type, 0, rand_ves_on_off);
        testing_the_system(stoch_on_off, rand_ves_on_off, CalC_on_off, par_init, CaExtracellular, save_data, savefilename, save_calc_loc, pVr2_hack, stim_freq, num_stim)
end
