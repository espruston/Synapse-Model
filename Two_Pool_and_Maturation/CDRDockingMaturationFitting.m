...
k_undocking = 0;
k_maturation = 0.006;
k_dematuration = 0.002;

C_3 = 10; %concentration of syt3
C_7 = 1; %concentration of syt7

CDR = .000965; %effect of calcium dependent recovery from depression
Facil = 0; %effect of facilitation
...
function dydt = dSS(~,state,k_docking,k_undocking,k_maturation,k_dematuration,syt3,CDR)
    ...
    dydt(1) = - state(1)*(k_docking + syt3*CDR) + state(2)*k_undocking;
    dydt(2) = state(1)*(k_docking + syt3*CDR) - state(2)*k_undocking - state(2)*k_maturation + state(3)*k_dematuration;
