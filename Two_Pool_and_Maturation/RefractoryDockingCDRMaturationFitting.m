...
% hold values, PPR R^2 = .91668, SS R^2 = .9223 train recovery R^2 = .94889, combined R^2 = .92929
% k_refractory = .9;
% k_docking = .002;
% k_undocking = 0;
% k_maturation = .002;
...
% 
% CDR = 30; %effect of calcium dependent recovery from depression
...
CDR = 9.65; %effect of calcium dependent recovery from depression
Facil = 0; %effect of facilitation
...
%[~,state] = ode45(@(t,state) dSS(t,state,k_refractory,k_docking,k_undocking,k_maturation,k_dematuration), [0 t_SS], state_0);
[~,state] = ode45(@(t,state) dSS(t,state,k_refractory,k_docking,k_undocking,k_maturation,k_dematuration,(Ca_rest^2/(K_D_3^2 + Ca_rest^2))*C_3, CDR), [0 t_SS], state_0);
...
function [ts, state, Fused, Ca_sim, syt3, syt7] = stim_sim(stimulus_times, max_time)
    ...
    for i = 1:length(stim_delay)
        ...
        [t,out] = ode45(@(t,state) dState(t,state,k_refractory,k_docking,k_undocking,k_maturation,k_dematuration, syt3, CDR), [0 stim_delay(i)], post_stim);
...
function [ISI, recovery, R_train_recovery, train_recovery_data, train_recovery_data_SEMs] = trainRecovery_fit()
    ...
    for j = 1:length(ISI)
        ...
        [~,out] = ode45(@(t,state) dState(t,state,k_refractory,k_docking,k_undocking,k_maturation,k_dematuration,syt3,CDR), [0 ISI(j)], post_train);
        recovery(j) = (out(end,3)*p_immature + out(end,4)*p_mature)/Fused(1);
...
function dydt = dSS(~,state,k_refractory,k_docking,k_undocking,k_maturation,k_dematuration,syt3,CDR)
    
    dydt(1) = -state(1)*k_refractory;
    dydt(2) = state(1)*k_refractory - state(2)*k_docking + state(3)*k_undocking;
    dydt(3) = state(2)*k_docking - state(3)*k_undocking - state(3)*(k_maturation + syt3*CDR) + state(4)*k_dematuration;
    dydt(4) = state(3)*(k_maturation + syt3*CDR) - state(4)*k_dematuration;
    dydt = transpose(dydt);

end


function dydt = dState(t,state,k_refractory,k_docking,k_undocking,k_maturation,k_dematuration,syt3,CDR)
...
%no syt effect hard coded
% function dydt = dSS(~,state,k_refractory,k_docking,k_undocking,k_maturation,k_dematuration)
%     
%     dydt(1) = -state(1)*k_refractory;
%     dydt(2) = state(1)*k_refractory - state(2)*k_docking + state(3)*k_undocking;
%     dydt(3) = state(2)*k_docking - state(3)*k_undocking - state(3)*k_maturation + state(4)*k_dematuration;
%     dydt(4) = state(3)*k_maturation - state(4)*k_dematuration;
%     dydt = transpose(dydt);
% 
% end
% 
% 
% function dydt = dState(~,state,k_refractory,k_docking,k_undocking,k_maturation,k_dematuration)
%     
...
%global referencing is slower here
% function dydt = dSS(t,state) %global referencing is slower than passing for SS calculation
%     
%     global k_refractory
%     global k_docking
%     global k_undocking
