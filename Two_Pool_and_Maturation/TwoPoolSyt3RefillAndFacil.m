...
% p_low = .1;
% 
% k_docking_high = 0.0015;
% k_docking_low = 0.0001;
% k_undocking_high = 0.00015;
% k_undocking_low = 0.00001;
% 
% 
% size_high = .2;
% size_low = 1-size_high;
...
function [ts, state, Fused_im, Fused_m, Ca_sim] = stim_sim(stimulus_times, max_time)
    ...
    for i = 1:length(stim_delay)
        ...
        [t,out] = ode23(@(t,state) dState(t,state,k_docking_low,k_undocking_low,k_docking_high,k_undocking_high,k_on_3,k_off_3,k_on_7,k_off_7,Ca_sim,delta_t,C_3), [ts(end) ts(end)+stim_delay(i)], post_stim);
        %using ode45 above appears to result in a weird bug where the fourth stimulus shows weird artifacts for syt concentrations
...
function dydt = dState(t,state,k_docking_low,k_undocking_low,k_docking_high,k_undocking_high,k_on_3,k_off_3,k_on_7,k_off_7,Ca_sim,delta_t,C_3)
