...
function [ts, state, Fused, Ca_sim] = stim_sim(stimulus_times, max_time)
    ...
    for i = 1:length(stim_delay)
        ...
        pre_stim = state(end,:);
        post_stim = pre_stim + [pre_stim(3)*p_low*C_3, pre_stim(4)*p_high, -pre_stim(3)*p_low*C_3, -pre_stim(4)*p_high];
        Fused(i) = pre_stim(3)*p_low*C_3 + pre_stim(4)*p_high;
        [t,out] = ode45(@(t,state) dState(t,state,k_docking_low,k_undocking_low,k_docking_high,k_undocking_high), [ts(end) ts(end)+stim_delay(i)], post_stim);
...
function dydt = dState(~,state,k_docking_low,k_undocking_low,k_docking_high,k_undocking_high)
