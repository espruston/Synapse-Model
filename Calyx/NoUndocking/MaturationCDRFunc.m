...
[t0,state] = ode15s(@(t,state) dSS(t,state,k_docking,k_maturation,k_dematuration,k_on_3,k_off_3,k_on_7,k_off_7,C_3,Ca_rest), [0 t_SS], state_0);

SS = state(end,:);

[~, ~, hz_1, ~, ~, hz_10, ~, ~, hz_20, ~, ~, hz_50, ~, ~, hz_100, ~, ~, hz_200] = test6(p_mature, p_immature, k_docking, C_3, k_maturation, k_dematuration, SS);

err = sqrt(sum((hz_1(1:20) - hz_1_data(1:20)).^2 + (hz_10(1:20) - hz_10_data(1:20)).^2 + (hz_20(1:20) - hz_20_data(1:20)).^2 + (hz_50(1:20) - hz_50_data(1:20)).^2 + (hz_100(1:20) - hz_100_data(1:20)).^2 + (hz_200(1:20) - hz_200_data(1:20)).^2));
  
function [ts, state, Fused_im, Fused_m, Ca_sim] = stim_sim(stimulus_times, max_time, p_mature, p_immature, k_docking, C_3, k_maturation, k_dematuration, SS)
    ...
    for i = 1:length(stim_delay)
        ...
        [t,out] = ode45(@(t,state) dState(t,state,k_docking,k_maturation,k_dematuration,k_on_3,k_off_3,k_on_7,k_off_7,C_3,Ca_sim), [ts(end) ts(end)+stim_delay(i)], post_stim);
...
function [Fused_im_1, Fused_m_1, hz_1, Fused_im_10, Fused_m_10, hz_10, Fused_im_20, Fused_m_20, hz_20, Fused_im_50, Fused_m_50, hz_50, Fused_im_100, Fused_m_100, hz_100, Fused_im_200, Fused_m_200, hz_200] = test6(p_mature, p_immature, k_docking, C_3, k_maturation, k_dematuration, SS)
    ...
    [~, ~, Fused_im_1, Fused_m_1,~] = stim_sim(stimulus_times_1, max_time_1, p_mature, p_immature, k_docking, C_3, k_maturation, k_dematuration, SS);
    Fused_1 = Fused_im_1 + Fused_m_1;
    hz_1 = Fused_1/Fused_1(1);
    
    max_time_10 = stimulus_times_10(end) + stimulus_times_10(2)*30;
    [~, ~, Fused_im_10, Fused_m_10,~] = stim_sim(stimulus_times_10, max_time_10, p_mature, p_immature, k_docking, C_3, k_maturation, k_dematuration, SS);
    Fused_10 = Fused_im_10 + Fused_m_10;
    hz_10 = Fused_10/Fused_10(1);
    
    max_time_20 = stimulus_times_20(end) + stimulus_times_20(2)*30;
    [~, ~, Fused_im_20, Fused_m_20,~] = stim_sim(stimulus_times_20, max_time_20, p_mature, p_immature, k_docking, C_3, k_maturation, k_dematuration, SS);
    Fused_20 = Fused_im_20 + Fused_m_20;
    hz_20 = Fused_20/Fused_20(1);

    max_time_50 = stimulus_times_50(end) + stimulus_times_50(2)*30;
    [~, ~, Fused_im_50, Fused_m_50, ~] = stim_sim(stimulus_times_50, max_time_50, p_mature, p_immature, k_docking, C_3, k_maturation, k_dematuration, SS);
    Fused_50 = Fused_im_50 + Fused_m_50;
    hz_50 = Fused_50/Fused_50(1);
    
    max_time_100 = stimulus_times_100(end) + stimulus_times_100(2)*100;
    [~, ~, Fused_im_100, Fused_m_100, ~] = stim_sim(stimulus_times_100, max_time_100, p_mature, p_immature, k_docking, C_3, k_maturation, k_dematuration, SS);
    ...
    [~, ~, Fused_im_200, Fused_m_200, ~] = stim_sim(stimulus_times_200, max_time_200, p_mature, p_immature, k_docking, C_3, k_maturation, k_dematuration, SS);
    Fused_200 = Fused_im_200 + Fused_m_200;
    hz_200 = Fused_200/Fused_200(1);

end

function dydt = dSS(~,state,k_docking,k_maturation,k_dematuration,k_on_3,k_off_3,k_on_7,k_off_7,C_3,Ca_rest)
    
    k_docking = k_docking*(1+state(4)*C_3);
    
    dydt(1,1) = -state(1)*k_docking;
    dydt(2,1) = state(1)*k_docking - state(2)*k_maturation + state(3)*k_dematuration;
...
function dydt = dState(t,state,k_docking,k_maturation,k_dematuration,k_on_3,k_off_3,k_on_7,k_off_7,C_3,Ca_sim)
