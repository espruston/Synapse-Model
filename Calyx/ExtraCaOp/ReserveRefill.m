...
x = [0.94853,0.0028148,0.00010118,10.4109,0.0001,1000000]; %best fit
%12/21 err=0.81545
...
%Syt3 KO
[t0,state] = ode15s(@(t,state) dSS(t, state, k_docking, k_undocking, Ca_rest*C_Ca, reserve_size, k_refill), [0 t_SS], state_0);
...
function [ts, state, Fused] = stim_sim(stimulus_times, max_time, p_release, k_docking, k_undocking, Ca, SS, reserve_size, k_refill)
    ...
    for i = 1:length(stim_delay)
        ...
        [t,out] = ode15s(@(t,state) dState(t, state, k_docking, k_undocking, Ca(1,:), reserve_size, k_refill), [ts(end) ts(end)+stim_delay(i)], post_stim);
    ...
    if stimulus_times == linspace(0,20*19,20) %50hz
        ...
        for i = 1:length(Recovery)
            ...
            [t,out] = ode15s(@(t,state) dState(t, state, k_docking, k_undocking, Ca(i,:), reserve_size, k_refill), stimulus_times(end)+[0 Recovery(i)], post_stim);
            pre_stim = out(end,:);
