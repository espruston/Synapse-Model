...
x = [0.8,0.0007,0.0001,10,0.0002,1e7];
...
%Syt3 KO
[t0,state] = ode23(@(t,state) dSS(t, state, k_docking, k_undocking, Ca_rest*C_Ca, reserve_size, k_refill), [0 t_SS], state_0);
...
function [ts, state, Fused] = stim_sim(stimulus_times, max_time, p_release, k_docking, k_undocking, Ca, SS, reserve_size, k_refill)
    ...
    for i = 1:length(stim_delay)
        ...
        [t,out] = ode23(@(t,state) dState(t, state, k_docking, k_undocking, Ca(1,:), reserve_size, k_refill), [ts(end) ts(end)+stim_delay(i)], post_stim);
    ...
    if stimulus_times == linspace(0,20*19,20) %50hz
        ...
        for i = 1:length(Recovery)
            ...
            [t,out] = ode23(@(t,state) dState(t, state, k_docking, k_undocking, Ca(i,:), reserve_size, k_refill), stimulus_times(end)+[0 Recovery(i)], post_stim);
            pre_stim = out(end,:);
