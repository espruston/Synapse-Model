...
[~,state] = ode113(@(t,state) dSS(t, state, k_docking, k_undocking, CF1HzSyt3(1)*C_3, reserve_size, k_docking_reserve), [0 t_SS], state_0);

SS = state(end,:);

% [~, ~, Fused_1] = stim_sim(stimulus_times_1, max_time_1, p_release, k_docking, k_undocking, CF1HzSyt3*C_3, C_7, CF1HzSyt7, SS, reserve_size, k_docking_reserve);
% hz_1 = Fused_1/Fused_1(1);
% 
% [~, ~, Fused_10] = stim_sim(stimulus_times_10, max_time_10, p_release, k_docking, k_undocking, CF10HzSyt3*C_3, C_7, CF10HzSyt7, SS, reserve_size, k_docking_reserve);
% hz_10 = Fused_10/Fused_10(1);
% 
% [~, ~, Fused_20] = stim_sim(stimulus_times_20, max_time_20, p_release, k_docking, k_undocking, CF20HzSyt3*C_3, C_7, CF20HzSyt7, SS, reserve_size, k_docking_reserve);
% hz_20 = Fused_20/Fused_20(1);
% 
% [~, ~, Fused_50] = stim_sim(stimulus_times_50, max_time_50, p_release, k_docking, k_undocking, CF50HzSyt3*C_3, C_7, CF50HzSyt7, SS, reserve_size, k_docking_reserve);
...
%Syt3 KO
C_3 = 0;
[t0,state] = ode113(@(t,state) dSS(t, state, k_docking, k_undocking, C_3, reserve_size, k_docking_reserve), [0 t_SS], state_0);
...
function [ts, state, Fused] = stim_sim(stimulus_times, max_time, p_release, k_docking, k_undocking, Syt3, C_7, Syt7, SS, reserve_size, k_docking_reserve)
    ...
    for i = 1:length(stim_delay)
        ...
        [t,out] = ode113(@(t,state) dState(t, state, k_docking, k_undocking, Syt3(1,:), reserve_size, k_docking_reserve), [ts(end) ts(end)+stim_delay(i)], post_stim);
    ...
    if stimulus_times == linspace(0,20*19,20)
        ...
        for i = 1:length(Recovery)
            ...
            [t,out] = ode113(@(t,state) dState(t, state, k_docking, k_undocking, Syt3(i,:), reserve_size, k_docking_reserve), stimulus_times(end)+[0 Recovery(i)], post_stim);
            pre_stim = out(end,:);
...
function dydt = dSS(~,state,k_docking,k_undocking,Syt3,reserve_size,k_docking_reserve)

dydt(1,1) = -state(1)*(state(3)/reserve_size)*k_docking*(1+Syt3) + state(2)*k_undocking;
dydt(2,1) = -dydt(1,1);
dydt(3,1) = dydt(1,1) + (reserve_size - state(3))*k_docking_reserve;

end

function dydt = dState(t,state,k_docking,k_undocking,Syt3,reserve_size,k_docking_reserve)
