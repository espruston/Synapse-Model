...
if exist('loaded','var') == 0
    ...
    %load Ca Signals
    CF1HzCa = matfile('CF1HzCa.mat').Ca_sim;
    CF10HzCa = matfile('CF10HzCa.mat').Ca_sim;
    CF20HzCa = matfile('CF20HzCa.mat').Ca_sim;
    CF50HzCa = matfile('CF50HzCa.mat').Ca_sim;
...
[~,state] = ode113(@(t,state) dSS(t, state, k_docking, k_undocking, CF1HzSyt3(1)*C_3, Ca_rest*C_Ca, reserve_size), [0 t_SS], state_0);

SS = state(end,:);

[~, ~, Fused_1] = stim_sim(stimulus_times_1, max_time_1, p_release, k_docking, k_undocking, CF1HzSyt3*C_3, C_7, CF1HzSyt7, CF1HzCa*C_Ca, SS, reserve_size);
hz_1 = Fused_1/Fused_1(1);

[~, ~, Fused_10] = stim_sim(stimulus_times_10, max_time_10, p_release, k_docking, k_undocking, CF10HzSyt3*C_3, C_7, CF10HzSyt7, CF10HzCa*C_Ca, SS, reserve_size);
hz_10 = Fused_10/Fused_10(1);

[~, ~, Fused_20] = stim_sim(stimulus_times_20, max_time_20, p_release, k_docking, k_undocking, CF20HzSyt3*C_3, C_7, CF20HzSyt7, CF20HzCa*C_Ca, SS, reserve_size);
hz_20 = Fused_20/Fused_20(1);

[~, ~, Fused_50] = stim_sim(stimulus_times_50, max_time_50, p_release, k_docking, k_undocking, CF50HzSyt3*C_3, C_7, CF50HzSyt7, CF50HzCa*C_Ca, SS, reserve_size);
...
%Syt3 KO
C_3 = 0;
[t0,state] = ode113(@(t,state) dSS(t, state, k_docking, k_undocking, C_3, Ca_rest*C_Ca, reserve_size), [0 t_SS], state_0);
...
function [ts, state, Fused] = stim_sim(stimulus_times, max_time, p_release, k_docking, k_undocking, Syt3, C_7, Syt7, Ca, SS, reserve_size)
    ...
    for i = 1:length(stim_delay)
        ...
        [t,out] = ode113(@(t,state) dState(t, state, k_docking, k_undocking, Syt3(1,:), Ca(1,:), reserve_size), [ts(end) ts(end)+stim_delay(i)], post_stim);
    ...
    if stimulus_times == linspace(0,20*19,20)
        ...
        for i = 1:length(Recovery)
            ...
            [t,out] = ode113(@(t,state) dState(t, state, k_docking, k_undocking, Syt3(i,:), Ca(i,:), reserve_size), stimulus_times(end)+[0 Recovery(i)], post_stim);
            pre_stim = out(end,:);
