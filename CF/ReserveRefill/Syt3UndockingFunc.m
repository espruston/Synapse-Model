function cost = Syt3UndockingFunc(x)
disp(['Testing x = [', num2str(x(1)), ',', num2str(x(2)), ',', num2str(x(3)), ',', num2str(x(4)), ',', num2str(x(5)), ',', num2str(x(6)), ']'])
...
[~, ~, Fused_1_WT] = stim_sim(stimulus_times_1, max_time_1, p_release, k_docking, k_undocking, SS_WT, reserve_size, k_refill, C_3, CF1HzSyt3);
hz_1_WT = Fused_1_WT/Fused_1_WT(1);

[~, ~, Fused_10_WT] = stim_sim(stimulus_times_10, max_time_10, p_release, k_docking, k_undocking, SS_WT, reserve_size, k_refill, C_3, CF10HzSyt3);
hz_10_WT = Fused_10_WT/Fused_10_WT(1);

[~, ~, Fused_20_WT] = stim_sim(stimulus_times_20, max_time_20, p_release, k_docking, k_undocking, SS_WT, reserve_size, k_refill, C_3, CF20HzSyt3);
hz_20_WT = Fused_20_WT/Fused_20_WT(1);

[~, ~, Fused_50_WT] = stim_sim(stimulus_times_50, max_time_50, p_release, k_docking, k_undocking, SS_WT, reserve_size, k_refill, C_3, CF50HzSyt3);
...
[~, ~, Fused_1_KO] = stim_sim(stimulus_times_1, max_time_1, p_release, k_docking, k_undocking, SS_KO, reserve_size, k_refill, C_3, CF1HzSyt3);
hz_1_KO = Fused_1_KO/Fused_1_KO(1);

[~, ~, Fused_10_KO] = stim_sim(stimulus_times_10, max_time_10, p_release, k_docking, k_undocking, SS_KO, reserve_size, k_refill, C_3, CF10HzSyt3);
hz_10_KO = Fused_10_KO/Fused_10_KO(1);

[~, ~, Fused_20_KO] = stim_sim(stimulus_times_20, max_time_20, p_release, k_docking, k_undocking, SS_KO, reserve_size, k_refill, C_3, CF20HzSyt3);
hz_20_KO = Fused_20_KO/Fused_20_KO(1);

[~, ~, Fused_50_KO] = stim_sim(stimulus_times_50, max_time_50, p_release, k_docking, k_undocking, SS_KO, reserve_size, k_refill, C_3, CF50HzSyt3);
...
function [ts, state, Fused] = stim_sim(stimulus_times, max_time, p_release, k_docking, k_undocking, SS, reserve_size, k_refill, C_3, Syt3)
    ...
    for i = 1:length(stim_delay)
        ...
        [t,out] = ode113(@(t,state) dState(t, state, k_docking, k_undocking, reserve_size, k_refill, C_3, Syt3(1,:)), [ts(end) ts(end)+stim_delay(i)], post_stim);
    ...
    if stimulus_times == linspace(0,20*19,20) %50hz
        ...
        for i = 1:length(Recovery)
            ...
            [~,out] = ode113(@(t,state) dState(t, state, k_docking, k_undocking, reserve_size, k_refill, C_3, Syt3(i,:)), stimulus_times(end)+[0 Recovery(i)], post_stim);
            pre_stim = out(end,:);
...
function dydt = dSS(~,state,k_docking,k_undocking,reserve_size,k_refill,C_3,Syt3)

dydt(1,1) = -state(1)*(state(3)/reserve_size)*k_docking + state(2)*k_undocking*C_3*(1-Syt3);
dydt(2,1) = -dydt(1,1);
...
function dydt = dState(t,state,k_docking,k_undocking,reserve_size,k_refill,C_3,Syt3)

dydt(1,1) = -state(1)*(state(3)/reserve_size)*k_docking + state(2)*k_undocking*C_3*(1-Syt3(round(t/.1)+1));
dydt(2,1) = -dydt(1,1);
dydt(3,1) = dydt(1,1) + (reserve_size-state(3))*k_refill;
