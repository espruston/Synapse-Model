x = [0.98286,0.00047238,0.002539,0.38457,5.9171]; %surrogateopt fit 11/9/2020
...
[~,state] = ode113(@(t,state) dSS(t,state,k_docking,k_undocking,CF1HzSyt3(1)*C_3), [0 t_SS], state_0);

SS = state(end,:);

[~, ~, Fused_2_1, Fused_1_1] = stim_sim(stimulus_times_1, max_time_1, p_release, k_docking, k_undocking, CF1HzSyt3*C_3, C_7, CF1HzSyt7, SS);
Fused_1 = Fused_2_1 + Fused_1_1;
hz_1 = Fused_1/Fused_1(1);

[~, ~, Fused_2_10, Fused_1_10] = stim_sim(stimulus_times_10, max_time_10, p_release, k_docking, k_undocking, CF10HzSyt3*C_3, C_7, CF10HzSyt7, SS);
Fused_10 = Fused_2_10 + Fused_1_10;
hz_10 = Fused_10/Fused_10(1);

[~, ~, Fused_2_20, Fused_1_20] = stim_sim(stimulus_times_20, max_time_20, p_release, k_docking, k_undocking, CF20HzSyt3*C_3, C_7, CF20HzSyt7, SS);
Fused_20 = Fused_2_20 + Fused_1_20;
hz_20 = Fused_20/Fused_20(1);

[~, ~, Fused_2_50, Fused_1_50] = stim_sim(stimulus_times_50, max_time_50, p_release, k_docking, k_undocking, CF50HzSyt3*C_3, C_7, CF50HzSyt7, SS);
...
%Syt3 KO
C_3 = 0;
[t0,state] = ode113(@(t,state) dSS(t,state,k_docking,k_undocking,C_3), [0 t_SS], state_0);
...
function [ts, state, Fused_2, Fused_1] = stim_sim(stimulus_times, max_time, p_release, k_docking, k_undocking, Syt3, C_7, Syt7, SS)
    ...
    for i = 1:length(stim_delay)
        ...
        [t,out] = ode113(@(t,state) dState(t,state,k_docking,k_undocking,Syt3(1,:)), [ts(end) ts(end)+stim_delay(i)], post_stim);
    ...
    if stimulus_times == linspace(0,50*19,20)
        ...
        for i = 1:length(Recovery)
            ...
            [t,out] = ode113(@(t,state) dState(t,state,k_docking,k_undocking,Syt3(i,:)), stimulus_times(end)+[0 Recovery(i)], post_stim);
            pre_stim = out(end,:);
...
function dydt = dSS(~,state,k_docking,k_undocking,Syt3)
    
    dydt(1,1) = -state(1)*k_docking + state(3)*k_undocking;
    dydt(2,1) = -state(2)*k_docking*Syt3 + state(4)*k_undocking;
    dydt(3,1) = -dydt(1,1);
    dydt(4,1) = -dydt(2,1);

end

function dydt = dState(t,state,k_docking,k_undocking,Syt3)
    
    dydt(1,1) = -state(1)*k_docking + state(3)*k_undocking;
    dydt(2,1) = -state(2)*k_docking*Syt3(round(t/.01)+1) + state(4)*k_undocking;
    dydt(3,1) = -dydt(1,1);
    dydt(4,1) = -dydt(2,1);
