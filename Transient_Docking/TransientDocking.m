...
[~,state] = ode45(@(t,state) dSS(t,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,0,0,Ca_rest/Ca_spike), [0 t_SS], state_0);
...
sq_sum_PPR = sum(((PPR-PPR_data)./PPR_data_SEMs./16).^2);
sq_sum_train_recovery = sum(((train_recovery-train_recovery_data)./train_recovery_data_SEMs./10).^2);
sq_sum_trainSS = sum(((trainSS-trainSS_data)./trainSS_data_SEMs./4).^2);
...
function dydt = dSS(~,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,syt3,syt7,Ca_res)
    ...
    dydt(1) = -state(1)*(k_docking + k_docking_transient*Ca_res) + state(2)*(k_undocking + k_undocking_transient*Ca_res);
    dydt(2) = state(1)*(k_docking + k_docking_transient*Ca_res) - state(2)*(k_undocking + k_undocking_transient*Ca_res);
    dydt = transpose(dydt);
...
function dydt = dState(t,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,syt3,syt7,Ca_sim_norm)
    ...
    dydt(1) = -state(1)*(k_docking + k_docking_transient*Ca) + state(2)*(k_undocking + k_undocking_transient*Ca);
    dydt(2) = state(1)*(k_docking + k_docking_transient*Ca) - state(2)*(k_undocking + k_undocking_transient*Ca);
    dydt = transpose(dydt);
