...
k_docking_transient = .005; %docking rate in presence of Ca-bound syt3
k_undocking_transient = .001; %basal undocking rate

k_docking = .002; %amount by which docking is decreased in the presence of Ca-unbound syt3
k_undocking = .007; %amount by which undocking is increaed in the presence of Ca-bound syt3
...
state_0 = [1; 0; 0; 0]; %all sites start empty
[~,state] = ode45(@(t,state) dSS(t,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,Ca_rest), [0 t_SS], state_0);
...
for j = 1:length(ISI_train_recovery)
    ...
    [t,out] = ode45(@(t,state) dState(t,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,Ca_sim), [0 ISI_train_recovery(j)], post_train);
...
disp(sum(((trainSS-trainSS_data_WT)/trainSS_data_SEMs_WT)) + sum(((PPR-PPR_data_WT)/PPR_data_SEMs_WT)) + sum(((train_recovery-train_recovery_data_WT)/train_recovery_data_SEMs_WT)))
...
function [ts, state, Fused, Ca_sim] = stim_sim(stimulus_times, max_time)
    ...
    for i = 1:length(stim_delay)
        ...
        [t,out] = ode45(@(t,state) dState(t,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,Ca_sim), [ts(end) ts(end)+stim_delay(i)], post_stim);
...
function dydt = dSS(~,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,Ca_rest)

    dydt(1) = -state(1)*(k_docking_transient - k_docking*(1-state(3))) + state(2)*(k_undocking_transient + k_undocking*state(3));
    dydt(2) = state(1)*(k_docking_transient - k_docking*(1-state(3))) - state(2)*(k_undocking_transient + k_undocking*state(3));
    
    dydt(3) = (1-state(3))*3e5*Ca_rest - state(3)*(.732*.244) - state(3)*(.266*.0537);
    dydt(4) = (1-state(4))*7.33e3*Ca_rest - state(4)*.011;
    dydt = transpose(dydt);
...
function dydt = dState(t,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,Ca_sim)
    ...
    dydt(3) = (1-state(3))*3e5*Ca - state(3)*(.732*.244) - state(3)*(.266*.0537);
    dydt(4) = (1-state(4))*7.33e3*Ca - state(4)*.011;
    dydt = transpose(dydt);
