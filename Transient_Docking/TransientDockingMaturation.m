...
% sq_sum_total = 39.7859
% p_immature = .1;
% p_mature = .8;
% 
% k_docking = .003; %amount by which docking is decreased in the presence of Ca-unbound syt3
% k_undocking = .003; %amount by which undocking is increaed in the presence of Ca-bound syt3
% 
% % k_docking_transient = .002; %docking rate in presence of Ca-bound syt3
% % k_undocking_transient = .007; %basal undocking rate
% 
% k_maturation = .009;
% k_dematuration = .003;
...
k_docking = .05; %amount by which docking is decreased in the presence of Ca-unbound syt3
k_undocking = .005; %amount by which undocking is increaed in the presence of Ca-bound syt3
...
state_0 = [1; 0; 0; 0; 0]; %all sites start empty
[~,state] = ode45(@(t,state) dSS(t,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,k_maturation,k_dematuration,Ca_rest,C_3,C_7), [0 t_SS], state_0);
...
[ts, state, Fused, Ca_sim] = stim_sim(stimulus_times, max_time);
...
Ca_sim = Ca_sim(stimulus_times(end)/delta_t:end);
train_recovery = zeros(1,length(ISI_train_recovery));

for j = 1:length(ISI_train_recovery)

    [t,out] = ode45(@(t,state) dState(t,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,k_maturation,k_dematuration,Ca_sim,C_3,C_7), [0 ISI_train_recovery(j)], post_train);
    train_recovery(j) = (out(end,2)*p_immature + out(end,3)*p_mature)/Fused(1);
...
trainSS_data_SEMs_WT = [0.013 0.00933 0.00759 0.00357];
...
function [ts, state, Fused, Ca_sim] = stim_sim(stimulus_times, max_time)
    ...
    for i = 1:length(stim_delay)
        ...
        [t,out] = ode45(@(t,state) dState(t,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,k_maturation,k_dematuration,Ca_sim,C_3,C_7), [ts(end) ts(end)+stim_delay(i)], post_stim);
...
function dydt = dSS(~,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,k_maturation,k_dematuration,Ca_rest,C_3,C_7)

    dydt(1) = -state(1)*(k_docking) + state(2)*(k_undocking);
    dydt(2) = state(1)*(k_docking) - state(2)*(k_undocking) - state(2)*k_maturation + state(3)*k_dematuration;
    dydt(3) = state(2)*k_maturation - state(3)*k_dematuration;
    
    dydt(4) = (1-state(4))*3e5*Ca_rest - state(4)*(.732*.244) - state(4)*(.266*.0537);
    dydt(5) = (1-state(5))*7.33e3*Ca_rest - state(5)*.011;
...
function dydt = dState(t,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,k_maturation,k_dematuration,Ca_sim,C_3,C_7)
    ...
    dydt(4) = (1-state(4))*3e5*Ca - state(4)*(.732*.244) - state(4)*(.266*.0537);
    dydt(5) = (1-state(5))*7.33e3*Ca - state(5)*.011;
