...
k_docking_transient = .07;
k_undocking_transient = .01;
...
[~,state] = ode45(@(t,state) dSS(t,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,(Ca_rest^2/(K_D_3^2 + Ca_rest^2))*C_3,(Ca_rest^2/(K_D_7^2 + Ca_rest^2))*C_7), [0 t_SS], state_0);
...
function dydt = dSS(~,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,syt3,syt7)
    ...
    dydt(1) = -state(1)*(k_docking + k_docking_transient*syt3) + state(2)*(k_undocking + k_undocking_transient*syt3);
    dydt(2) = state(1)*(k_docking + k_docking_transient*syt3) - state(2)*(k_undocking + k_undocking_transient*syt3);
    dydt = transpose(dydt);
