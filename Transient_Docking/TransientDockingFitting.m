...
docking = linspace(0.0001, 0.01, 100);
undocking = linspace(0.0001, 0.01, 100);

for m = 1:100
    disp(m)
    for n = 1:100
        k_docking = docking(m);
        k_undocking = undocking(n);

        t_SS = 10000;
        SS = [k_docking/(k_docking+k_undocking), k_undocking/(k_docking+k_undocking)];
        ...
        if sq_sum_total < best(1)
            best = [sq_sum_total, k_docking, k_undocking];
        end
...
function dydt = dSS(~,state,k_docking,k_undocking,k_docking_transient,k_undocking_transient,syt3,syt7)
    ...
    dydt(1) = -state(1)*(k_docking + k_docking_transient*syt7) + state(2)*(k_undocking + k_undocking_transient*syt7);
    dydt(2) = state(1)*(k_docking + k_docking_transient*syt7) - state(2)*(k_undocking + k_undocking_transient*syt7);
    dydt = transpose(dydt);
