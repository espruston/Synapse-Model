...
global p_release
global k_docking_1
global k_undocking_1
global k_docking_2
global k_undocking_2
global amp_1
global amp_2
...
[~,state] = ode45(@(t,state) dSS(t,state,k_docking_1,k_undocking_1,k_docking_2,k_undocking_2,amp_1,amp_2,k_docking_transient,k_undocking_transient,0,(Ca_rest^2/(K_D_7^2 + Ca_rest^2))*C_7), [0 t_SS], state_0);
...
for j = 1:length(ISI_train_recovery)
    ...
    [t,out] = ode45(@(t,state) dState(t,state,k_docking_1,k_undocking_1,k_docking_2,k_undocking_2,amp_1,amp_2,k_docking_transient,k_undocking_transient,syt3,syt7), [0 ISI_train_recovery(j)], post_train);
...
% stimulus_times = [0 10000];
% max_time = 10500;
% 
% [t, state, Fused] = stim_sim(stimulus_times, max_time);
% plot(t,state(:,2));
...
function [ts, state, Fused] = stim_sim(stimulus_times, max_time)
    ...
    for i = 1:length(stim_delay)
        ...
        [t,out] = ode45(@(t,state) dState(t,state,k_docking_1,k_undocking_1,k_docking_2,k_undocking_2,amp_1,amp_2,k_docking_transient,k_undocking_transient,syt3,syt7), [ts(end) ts(end)+stim_delay(i)], post_stim);
...
function dydt = dSS(~,state,k_docking_1,k_undocking_1,k_docking_2,k_undocking_2,amp_1,amp_2,k_docking_transient,k_undocking_transient,syt3,syt7)

    
    dydt(1) = -state(1)*(amp_1*(k_docking_1 + k_docking_transient*syt7) + amp_2*(k_docking_2 + k_docking_transient*syt7)) + state(2)*(amp_1*(k_undocking_1 + k_undocking_transient*syt7) + amp_2*(k_undocking_2 + k_undocking_transient*syt7));
    dydt(2) = state(1)*(amp_1*(k_docking_1 + k_docking_transient*syt7) + amp_2*(k_docking_2 + k_docking_transient*syt7)) - state(2)*(amp_1*(k_undocking_1 + k_undocking_transient*syt7) + amp_2*(k_undocking_2 + k_undocking_transient*syt7));
    dydt = transpose(dydt);

end


function dydt = dState(t,state,k_docking_1,k_undocking_1,k_docking_2,k_undocking_2,amp_1,amp_2,k_docking_transient,k_undocking_transient,syt3,syt7)
