...
function result_mat = Vesicle_maturation_model_func(frequency, T_refill, T_1_2, T_facilitation, p_release_1, p_release_2, p_release_3, n_vesicles, n_pulses, cpv)
    ...
    n_s0 = 0; %no available docking sites
    ...
    current = 0; %abstract current of unit -1 per vesicle (stage independent)

    result_mat = zeros(n_pulses + 1, 14); %matrix with values per pulse

    for i = 1:n_pulses

        n_s1 = n_s1 - n_released_1 - n_s2_new + n_s1_new; %new total number of t1 vesicles at pulse i+1
